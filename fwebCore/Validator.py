...
def validate_article(article):
    ...
    if not v_date(date):
        ...
        if v_title(title) and v_body(body):
            ...
            newArticle = DICT.replace_key_value(article, "published_date", DATE.mongo_date_today())
            return newArticle
...
def save_article(article):
    return dbArticles.ADD_ARTICLES(article)
...
def mongo_save(func):
    ...
    def wrapper(*args):
        ...
        if article:
            ...
            dbArticles.ADD_ARTICLES(article)
            return True
        return False
    return wrapper